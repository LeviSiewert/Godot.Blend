...
class SubresourceCollection[T:GdSubResource](Collection, GdType):
    @classmethod
    def lark_keys(cls):
        return ("sub_resources",)
    @classmethod
    def parse_lark(cls, key, tfm, *children):
        self = cls()
        self.extend(filter(lambda x: not(x is None), children))
        return self
...
class EditresourceCollection[T:GdEditResource](SubresourceCollection):
    @classmethod
    def lark_keys(cls):
...
class ExtresourceCollection[T:GdExtResource](SubresourceCollection):
    @classmethod
    def lark_keys(cls):
...
class GdResourceFileTres(GdResource, ClassDbEnforcable):
    ...
    @classmethod
    def parse_lark(cls, key:str, tfm, header_props:PropertyCollection, ext_res:ExtresourceCollection, sub_res:SubresourceCollection, prim_resource:PropertyCollection):
        self = cls(True)
...
class GdResourceFileScene(GdResource):
    ...
    properties : PropertyCollection
    ext_resources : SubresourceCollection[GdExtResource]
    sub_resources : SubresourceCollection[GdSubResource]
    node_resources : SubresourceNodeCollection[GdSubResourceNode]
    ...
    @classmethod
    def parse_lark(cls, key:str, tfm, header_props:PropertyCollection, ext_res:ExtresourceCollection, sub_res:SubresourceCollection, node_res:SubresourceNodeCollection, edit_res:EditresourceCollection):
        self = cls(True)
        # raise Exception(sub_res)
    ...
    def __init__(self, _construct=False):
        if not _construct:
            ...
            self.properties = PropertyCollection()
            self.sub_resources = SubresourceCollection()
            self.node_resources = SubresourceNodeCollection()
            self.ext_resources = ExtresourceCollection()
    ...
    def get_struct_children(self)->list:
        ...
        res.extend(self.header_props.values())
        res.extend(self.sub_resources.values())
        res.extend(self.node_resources.values())
        res.extend(self.ext_resources.values())
...
class GdSubResource(GdResource, ClassDbEnforcable):
    ...
    @classmethod
    def parse_lark(cls, key, tfm, header_props:PropertyCollection, resource_body:PropertyCollection):
...
_all : tuple[Type] = (
    ...
    SubresourceNodeCollection,
    SubresourceCategoryCollection,
