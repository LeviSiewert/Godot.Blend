...
class Test_Project_Fs():
    ...
    def test_defered_write_import(self,):
        ...
        assert prj.files["mem://test.txt"] is txt_file

        ## Test _defered_write:
        assert file_system.read_text("mem://test.txt") == "uid://abc"
        
        ## Test _defered_import:
        assert txt_file.data == "uid://abc"
    ...
    def test_search():
        ...
        ## Assert discovery and construction:
        assert len(prj.files) == 1

        file = prj.files["mem://test.txt"]
        assert isinstance(file, FileTxt)

        file.read()
        assert file.data == "file_contents"

        file_system.close()


    def test_defered_exp_imp_known_res(self,):
        ''' Test defered write & import on known resource+file. 
        Resource should not be replaced as it's known and up to date (just exported) '''
        file_system = MemoryFileSystem()
        
        resource = ResourceTres.construct(
            "uid://abc",
        )
        
        res_file = FileTres.construct(
            "mem://test.tres",
            data = resource,
            _defered_write=True,
            _defered_import=True,
        ...
        assert prj.resources["uid://abc"] is resource
        assert prj.files["mem://test.tres"] is res_file

        assert res_file.data.get() is resource
        assert resource.file.get() is res_file
